...
(function theme() {
  ...
  const applyTheme = (light) => {
    ...
    toggle.setAttribute('aria-label', light ? 'Cambiar a tema oscuro' : 'Cambiar a tema claro');
  };
...
if (toggleBtn && nav) {
  toggleBtn.addEventListener('click', () => {
    ...
    if (!open && dropdown) dropdown.classList.remove('open');
...
if (dropdown && dropdownToggle) {
  dropdownToggle.addEventListener('click', () => {
    if (isMobile()) dropdown.classList.toggle('open');
  });
}
...
/* ============================================================
   Contador de experiencia total acumulada
   Calcula la diferencia entre cada fecha de inicio y hoy.
   Z-Bombilla: ene 2022 → actualidad (se actualiza cada día)
   Difusión Comunicación: ene 2021 – dic 2021 (12 meses)
   Alto Rendimiento: abr 2018 – jun 2018 (3 meses)
   ============================================================ */
(function experienceCounter() {
  const elY = document.getElementById('expYears');
  const elM = document.getElementById('expMonths');
  const elD = document.getElementById('expDays');
  if (!elY || !elM || !elD) return;
  ...
  const calc = () => {
    const zBombilla = toMonths(new Date(2022, 0));   // ene 2022 → hoy
    const difusion = 12;                              // ene–dic 2021
    const altoRendimiento = 3;                        // abr–jun 2018
    const totalMonths = zBombilla + difusion + altoRendimiento;
    ...
    const d = Math.floor(diffMs / 86400000) % 30;
